...
$(window).on('load', function() {
    ...
    // Set the default background to the GIF (bg.gif)
    $('#main').css("background-image", "url(bg.gif)");
    ...
    if (
        ...
        while (name.length > 8) {
            alert('Keep the Name length to 8 characters or less');
        ...
        document.getElementById('name').innerHTML = localStorage.getItem('name');
        let wallpaper = localStorage.getItem('wp');
        if (wallpaper.endsWith('.gif')) {
            // If the wallpaper is a GIF, use it as a background
            $('#main').css('background-image', 'url(img/wallpapers/' + wallpaper + ')');
        } else {
            // If it's not a GIF, assume it's an image
            $('#main').css('background-image', 'url(img/wallpapers/' + wallpaper + '.jpg)');
        }
    }
});
